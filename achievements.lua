...
achievements.Slingshot = {name = "Slingshot",hits = 0, targetHits = 8, delay = 0.2, cooldown = 0}
achievements.Dunked = {name = "Dunked",hitPlayer = nil, hitLaser = nil, lost = nil,delay = 0.2,cooldown = 0}
...
function achievements:getAchieved()
	...
	if self.Slingshot.hits >= self.Slingshot.targetHits then
		table.insert(achieved,self.Slingshot)
	...
	if self.Dunked.lost ~= nil and self.Dunked.hitPlayer ~= self.Dunked.hitLaser and self.Dunked.hitLaser == self.Dunked.lost then
		table.insert(achieved,self.Dunked)
		self.Dunked.hitPlayer = nil
		self.Dunked.hitLaser = nil
		self.Dunked.lost = nil
		self.Dunked.cooldown = 0
	end
	if self.Disciplined.counter[1] > self.Disciplined.time or self.Disciplined.counter[2] > self.Disciplined.time then
		table.insert(achieved,self.Disciplined)
	end
	if self.Sniper.sniped ~= nil then
		table.insert(achieved,self.Sniper)
	end
	return achieved
end
